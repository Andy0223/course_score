...
<?php
    ...
    $rs1 = mysqli_query($conn,$sql_select);
    
    if($record = mysqli_fetch_row($rs1))
    {
        $Name = $record[1];
        $Department = $record[2];
        $Email = $record[3];
...
<body>
  ...
  <input type="hidden" name="method" value="update_Level">
   <section class="page-section" id="table">
    <div class="container">
      <div class="row">
        <div class="col-lg-12 text-center">
          <h2 class="section-heading text-uppercase">修改帳號</h2>
          <h3 class="section-subheading text-muted">Account Modify</h3>
        ...
        <table id="tb" style="border-collapse:collapse" align="center" cellpadding="10">
            ...
            <tr>
                ...
                <td><input type="text" name="department" id="password" value="<?php echo $Department?>"></td>
                <td><input type="email" name="email" id="email" value="<?php echo $Email?>"></td>
                <td><input type="text" name="identification" id="identification" value="<?php echo $Identification?>"></td>
                <td><select id= "level" name="level"><option value="請選擇">請選擇<option value="manager">管理者</option><option value="member">一般會員</option></select></td>
            </tr>
                
     </table>
        <br><br>
         <input type= 'submit' class="btn btn-primary" onclick="myfunction()">&emsp;
